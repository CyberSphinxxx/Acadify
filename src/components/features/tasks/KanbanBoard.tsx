...
import { TaskCard } from './TaskCard'; // For overlay
...
export function KanbanBoard({ tasks }: KanbanBoardProps) {
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (newStatus && newStatus !== activeTask.status) {
            // Update in Firestore
            try {
                // Optimistic update could happen here if we managed local state deeply, 
                // but since we subscribe to Firestore, we'll let the subscription update the UI.
                // However, DndKit likes immediate feedback. For now, rely on quick Firestore roundtrip.
